...
void ProcesadorComandos::grafoDePalabras(std::list<Palabra>* palabras, Graph* grafoPalabras){
  grafoPalabras->addVertices(palabras);
  grafoPalabras->addConnections();
}

void ProcesadorComandos::agregarPalabrasGrafo(std::list<Palabra>* palabras, Graph* grafoPalabras){
     std::list<Palabra>::iterator it;
    for (it = palabras->begin(); it != palabras->end(); ++it){
        grafoPalabras->addVertex(it->stringPalabra());
    }
}
...
bool ProcesadorComandos::esValida(std::string palabra){
    int contador = 0;
  for(int i = 0; i < palabra.length(); i++){
        if (!isalpha(palabra[i])){
            if(palabra[i] != '?'){
