...
void Sistema::recibirComando(char *comando) {
    while (true) {
        ...
        ProcesadorComandos::grafoDePalabras(diccionarioNormal.getPalabrasRef(), &grafoPalabras);
        std::cout<<"Grafo inicializado correctamente"<<std::endl;
        } else if (strcmp(comando1, "posibles_palabras") == 0) {
            if(!ProcesadorComandos::esValida(comando2)){
                std::cout<<"La cadena letras contiene símbolos inválidos"<<std::endl;
            }
            else{
                std::string cadenaLetras(comando2);
                cadenaLetras = ProcesadorComandos::enMayuscula(cadenaLetras);
                grafoPalabras.sinComodin(cadenaLetras);
            }
